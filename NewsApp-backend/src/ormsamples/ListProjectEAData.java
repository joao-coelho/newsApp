...
public class ListProjectEAData {
	...
	public void listTestData() throws PersistentException {
		...
		newsapp.Channel[] newsappChannels = ChannelDAO.listChannelByQuery(null, null);
		int length = Math.min(newsappChannels.length, ROW_COUNT);
		...
		newsapp.Article[] newsappArticles = ArticleDAO.listArticleByQuery(null, null);
		...
		newsapp.Comment[] newsappComments = CommentDAO.listCommentByQuery(null, null);
		...
		newsapp.Suggestion[] newsappSuggestions = SuggestionDAO.listSuggestionByQuery(null, null);
		...
		newsapp.ChannelSuggestion[] newsappChannelSuggestions = ChannelSuggestionDAO.listChannelSuggestionByQuery(null, null);
		...
		newsapp.ArticleSuggestion[] newsappArticleSuggestions = ArticleSuggestionDAO.listArticleSuggestionByQuery(null, null);
		...
		newsapp.User[] newsappUsers = UserDAO.listUserByQuery(null, null);
		length = Math.min(newsappUsers.length, ROW_COUNT);
		for (int i = 0; i < length; i++) {
		...
		newsapp.ChannelCategory[] newsappChannelCategorys = ChannelCategoryDAO.listChannelCategoryByQuery(null, null);
		...
		newsapp.Category[] newsappCategorys = CategoryDAO.listCategoryByQuery(null, null);
		...
		newsapp.ArticleCategory[] newsappArticleCategorys = ArticleCategoryDAO.listArticleCategoryByQuery(null, null);
	...
	public void listByCriteria() throws PersistentException {
		...
		int length =newsappChannels== null ? 0 : Math.min(newsappChannels.length, ROW_COUNT); 
		for (int i = 0; i < length; i++) {
			 System.out.println(newsappChannels[i]);
		}
		...
		length =newsappUsers== null ? 0 : Math.min(newsappUsers.length, ROW_COUNT); 
		for (int i = 0; i < length; i++) {
			 System.out.println(newsappUsers[i]);
		}
